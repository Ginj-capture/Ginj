...
public class BasicTimelineSliderUI extends TimelineSliderUI {
    ...
    protected Rectangle activeRect = null;
    ...
    /**
     * Returns the color of the active area.
    ...
    protected void installDefaults( JTimelineSlider slider ) {
        ...
        activeColor = UIManager.getColor("Slider.active");
    ...
    protected void calculateFocusRect() {
        ...
        System.out.println(focusRect + " for timeline");
    }
    ...
    protected void calculateActiveRect() {
        ...
        activeRect.x = lowerPosition;
        activeRect.width = higherPosition - lowerPosition;
        activeRect.y = trackRect.y;
        activeRect.height = activeTrackImage.getHeight();
    }
    /**
...
//        return new Dimension(thumbImage.getWidth(), thumbImage.getHeight());
    ...
    public void paint( Graphics g, JComponent c )   {
        ...
        if ( slider.getPaintTrack() && clip.intersects( activeRect ) ) {
            paintActive( g );
...
//        }
    ...
    public void paintActive(Graphics g)  {
        Rectangle activeBounds = activeRect;
        Rectangle trackBounds = trackRect;

        if ( slider.getOrientation() == JTimelineSlider.HORIZONTAL ) {
            int cy = (trackBounds.height / 2) - 2; // draw active inside track
            int cw = activeBounds.width;

            g.translate(activeBounds.x, trackBounds.y + cy);

            g.setColor(getActiveColor());
            g.drawLine(1, 2, cw-1, 2);
            g.drawLine(1, 3, cw-1, 3);

            g.translate(-activeBounds.x, -(trackBounds.y + cy));
        }
        else {
            int cx = (trackBounds.width / 2) - 2; // draw active inside track
            int ch = activeBounds.height;

            g.translate(trackBounds.x + cx, activeBounds.y);

            g.setColor(getActiveColor());
            g.drawLine(2, 1, 2, ch-1);
            g.drawLine(3, 1, 3, ch-1);

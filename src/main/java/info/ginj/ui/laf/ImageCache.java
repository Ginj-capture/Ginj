 ...
 *
 * Copied from sun.swing.ImageCache
...
public class ImageCache {
    ...
    private ImageCache.Entry getEntry(Object key, GraphicsConfiguration config,
                                      int w, int h, Object[] args) {
        ImageCache.Entry entry;
        Iterator<SoftReference<Entry>> iter = entries.listIterator();
        while (iter.hasNext()) {
            SoftReference<ImageCache.Entry> ref = iter.next();
            entry = ref.get();
            if (entry == null) {
        ...
        // Entry doesn't exist
        entry = new ImageCache.Entry(config, w, h, args);
    ...
    public Image getImage(Object key, GraphicsConfiguration config,
                          int w, int h, Object[] args) {
        ImageCache.Entry entry = getEntry(key, config, w, h, args);
